...
toronto_other_columns <-  mutate_all(merged_df[,4:8], as.numeric)
...
#Create graph2 dataframe
graph2_df <- filter(merged_df, neighbourhood %in% c(neighborhoods))
graph2_df <- add_row(graph2_df)

#Assign values to third row
graph2_df[3,2:3] <- c('Toronto', toronto_median_income)
graph2_df[3,4:8] <- toronto_other_columns
graph2_df
...
ggplot(graph2_df, aes(x = neighborhoods, y = c(no_certificate_diploma_or_degree, college_cegep_or_other_non_university_certificate_or_diploma, university_certificate_or_diploma_below_bachelor_level))) + 
  # Specify that we want a bar graph:
  geom_col() + 
  # Add titles and axis labels:
  labs(title = "Neighborhoods with the Highest and Lowest median income, \nand Toronto median income", 
