...
```{r echo= TRUE, fig1, fig.height = 3, fig.width = 9, size = "tiny"}
#Plotting the graph
ggplot(graph1_df_reshaped, aes(x = neighbourhood, y = as.numeric(income_value), fill = income_type)) + 
  ...
  labs(title = 
         "Neighbourhoods with the Highest and Lowest median income, and Toronto median income", 
       x = "neighbourhood", 
       y = "Income") 
```
...
#Plotting the graph
ggplot(
graph2_df_percentage_long, aes(x = neighbourhood, y = value, 
                               fill = study_level)) + 
  # Specify that we want a bar graph:
  geom_bar(stat="identity", position=position_dodge()) +
  # Add titles and axis labels:
